...
// Camera camera(glm::vec3(0.0f, 0.0f, 12.0f), glm::vec3(0.0f, 1.0f, 0.0f),
// -90.0f, 0.0f);
...
int main(void) {
  ...
  // Set up glfw context and window
  {
    ...
    window = glfwCreateWindow(SCREEN_WIDTH, SCREEN_HEIGHT,
                              "Polygon triagulation with monotone partitioning",
                              NULL, NULL);
    if (!window) {
      glfwTerminate();
      return -1;
  ...
  VertexBuffer VBO_edge(original_poly_edge_data.data(),
                        original_poly_edge_data.size());
  VAO_original_poly_edges.addVertexLayout(0, 2, GL_FALSE, 2 * sizeof(float), 0);

  VertexArray VAO_monotone_poly_edges;
  VertexBuffer VBO_mono_edge(monotone_poly_edge_data.data(),
                             monotone_poly_edge_data.size());
  ...
  VertexBuffer VBO_tri_ear_edge(trigulation_ear_clipping.data(),
                                trigulation_ear_clipping.size());
  VAO_triangulation_ear_edges.addVertexLayout(0, 2, GL_FALSE, 2 * sizeof(float),
                                              0);

  VertexArray VAO_triangulation_faces;
  VertexBuffer VBO_tri_faces(trigulation_face_data.data(),
                             trigulation_face_data.size());
  VAO_triangulation_faces.addVertexLayout(0, 3, GL_FALSE, 6 * sizeof(float), 0);
  VAO_triangulation_faces.addVertexLayout(1, 3, GL_FALSE, 6 * sizeof(float),
                                          (3 * sizeof(float)));

  ShaderProgram poly2dshader("./basic_poly_2d.shader");
  ShaderProgram shader("./triangle2d.shader");
  ShaderProgram line_shader("./generic_line.shader");
  ...
  bool show_monotone_partitioning = false;
  bool show_tri_faces = false;
  bool show_tri_ear = false;
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    if (show_tri_faces) {
      ...
      glDrawArrays(GL_TRIANGLES, 0, trigulation_face_data.size() / 6);
    }
    ...
    if (show_tri_ear) {
      ...
      glLineWidth(2);
      glDrawArrays(GL_LINES, 0, trigulation_ear_clipping.size() / 2);
    }
    ...
    ImGui::Begin(
        " Sample : Polygon triangulation to monotone pieces and Polygon "
        "triangulation");
