...
int main(void)
{
	...
	//Set up glfw context and window
	{
		...
		window = glfwCreateWindow(SCREEN_WIDTH, SCREEN_HEIGHT, "Quad Tree Visualization", NULL, NULL);
		if (!window)
		{
			glfwTerminate();
			return -1;
	...
	std::vector<jmk::Segment2d> line_vecs;
	std::vector<float> line_data;

	std::vector<jmk::Segment2d> balanced_line_vecs;
	...
	jmk::QuadTree tree(points, bounds);
	tree.GetUniqueSegmentList(line_vecs);
	...
	VertexArray VAO__balanced_lines;
	VertexBuffer VBO_blanced_lines(balanced_line_data.data(), balanced_line_data.size());
	VAO_points.addVertexLayout(0, 2, GL_FALSE, 2 * sizeof(float), 0);
	...
	bool show_points = true, show_split_lines = true, show_balanced_lines = false;
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		ImGui::Begin(" Sample : Quad Tree Visualization");
		ImGui::Checkbox("Points", &show_points);
		ImGui::Checkbox("Show Split Lines", &show_split_lines);
		ImGui::Checkbox("Show balanced Lines", &show_balanced_lines);
