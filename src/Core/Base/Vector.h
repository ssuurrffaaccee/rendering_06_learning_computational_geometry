...
#define Z 2
	...
	class Vector 
	{
		...
		template<typename _coordinate_type, size_t _dimensions>
		friend float dotProduct(const Vector<_coordinate_type, _dimensions>& v1, const Vector<_coordinate_type, _dimensions>& v2);
	...
	float dotProduct(const Vector<coordinate_type, dimensions>& v1, const Vector<coordinate_type, dimensions>& v2)
	{
		if (v1.coords.size() != v2.coords.size())
			return Limits<coordinate_type>::MIN;
